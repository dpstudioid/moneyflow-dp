...
const Login = () => {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      const msg = err.code === "auth/invalid-credential" ? "Email atau kata sandi salah"
  ...
  const handleGoogle = async () => {
    try {
      ...
      if (err.code !== "auth/popup-closed-by-user") {
        toast({ title: "Gagal masuk dengan Google", description: err.message, variant: "destructive" });
      }
    }
  };
      ...
      <div className="min-h-screen flex flex-col items-center justify-center px-4 bg-background">
        <div className="w-full max-w-sm space-y-8">
          <div className="text-center space-y-2">
            ...
            <p className="text-muted-foreground text-sm">Kelola keuangan rumah tangga bersama</p>
          </div>
            ...
            <CardContent className="pt-6 space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Input id="password" type={showPassword ? "text" : "password"} placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleLogin()} />
                  <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground">
                    {showPassword ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                  </button>
                </div>
              </div>
              <Button className="w-full font-semibold" size="lg" onClick={handleLogin} disabled={loading}>
                {loading ? <span className="animate-spin w-4 h-4 border-2 border-primary-foreground border-t-transparent rounded-full" /> : "Masuk"}
              </Button>
